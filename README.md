...
## 七、扩展计划  
- 🚀 **多线程支持**：基于 `clone()` 系统调用实现用户级线程库（含线程调度、同步原语）。  
- ⚡ **调度优化**：替换轮转调度为 **优先级调度** 或 **CFS 调度**，观察性能变化。  
- 📁 **文件系统增强**：实现 **日志（Journaling）** 或 **目录缓存**，提升文件读写效率。  
- 🌐 **网络实验**：为 xv6 添加虚拟网络驱动（如 NE2000 模拟），支持简单 TCP/IP 通信。  


## 八、贡献指南  
欢迎通过以下方式参与项目：  
1. **代码注释**：为 xv6 核心代码添加中文注释（如 `proc.c` `vm.c`），帮助更多人理解。  
2. **实验方案**：提交 MIT 6.828 实验的解决方案（如系统调用、内存管理实验）。  
3. **文档完善**：补充模块解析、调试技巧、扩展思路等内容。  

**提交规范**：  
- 遵循 xv6 原有代码风格（K&R 缩进，函数简洁）。  
- 提交 PR 时，附加实验步骤或注释说明，确保可复现。  


## 九、常见问题  
1. **QEMU 启动报错**：  
   → 检查交叉编译工具链是否正确（`riscv64-linux-gnu-gcc`），或尝试更新 QEMU 版本（`sudo apt upgrade qemu`）。  

2. **GDB 无法连接**：  
   → 确保开启 `make qemu-gdb` 后，再启动 GDB 并执行 `target remote localhost:26000`。  

3. **代码修改后编译失败**：  
   → 执行 `make clean` 清除旧编译产物，再重新 `make`。  

（本项目基于 MIT xv6 2020 版本，遵循 [MIT License](https://github.com/mit-pdos/xv6-riscv/blob/master/LICENSE)。）
